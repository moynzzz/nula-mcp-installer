...
4. **Merges** the `nula` MCP server block into the right top-level key of the right JSON file — preserving every other MCP server, every unrelated key, and every `_comment_*` field you may have added. Atomic write via temp file + rename.
...
Paste these snippets into the relevant file, replacing `https://YOUR_NULA_INSTANCE/mcp` with your host. If the file already has `mcpServers` / `mcp`, add the `nula` server *inside* that block — don't overwrite.
...
```json
{
  "mcpServers": {
    "nula": {
      ...
      "tools": ["*"],
      "headers": {
        "Authorization": "Bearer ${NULA_MCP_TOKEN}"
      }
    }
  }
}
...
> Copilot CLI does NOT do OAuth automatically. Mint a bearer token externally (the Nula repo ships `scripts/qa-mcp-flow.sh`) and export it as `NULA_MCP_TOKEN` before starting `copilot`. Tokens expire after 90 days.
...
```sh
opencode mcp auth nula
opencode mcp debug nula    # verify
```
...
Your `NULA_MCP_TOKEN` is missing, expired, or for the wrong instance. Mint a fresh one (`scripts/qa-mcp-flow.sh` from the Nula repo) and `export NULA_MCP_TOKEN=...` before starting `copilot`.
...
- Token-bearing config (Copilot CLI) uses an env-var reference (`${NULA_MCP_TOKEN}`), not the token itself. Your config files never contain the literal bearer.
